...
def collect_ks_stats(M, p, mu, alpha, beta, T, n, type, theta_factor=0.9, level=0.05, peri=2.0, epsilon=0.3):
    """
    ...
    for _ in range(M):
        ...
        for i in range(n):
            ts = thinning_hawkes(mu, alpha, beta, T, kernel_type=type, peri=peri, epsi=epsilon).simulate()
            flat_all.extend(ts)
            if i in idx_set:
...
def run_and_save_experiment(param_name, param_values, fixed_params, kernels, M, p, n, T, alpha_sig=0.05):
    ...
    for kernel in kernels:
        ...
        plt.plot(param_values, proportions, marker='s', markersize=8, linestyle='-', label='Rejection Proportion')
        plt.axhline(y=alpha_sig, linestyle='--', label=f'Alpha Level ({alpha_sig})')
        plt.xlabel(f"Parameter Value: {param_name}", fontsize=12)
        plt.ylabel("Proportion of Rejecting H0", fontsize=12)
        plt.title(f"Sensitivity Analysis: {param_name} vs Rejection Rate\n[Kernel: {kernel}, M={M}, n={n}]", fontsize=14)
        plt.ylim(-0.05, 1.05)
        plt.legend()
